package com.cj.shopping.recommendate.util;
...
public class RecommendUtils {
     ...
     */
    public static ConcurrentHashMap<Long, ConcurrentHashMap<Long, Long>> assembleUserBehavior(List<UserActiveDAO> userActiveList) {
        ConcurrentHashMap<Long, ConcurrentHashMap<Long, Long>> activeMap = new ConcurrentHashMap<Long, ConcurrentHashMap<Long, Long>>();
        // 遍历查询到的用户点击行为数据
        for (UserActiveDAO userActive : userActiveList) {
            // 1.获取用户id
     ...
     */
    public static List<UserSimilarityDAO> calcSimilarityBetweenUsers(ConcurrentHashMap<Long, ConcurrentHashMap<Long, Long>> activeMap) {
        // 用户之间的相似度对集合
        List<UserSimilarityDAO> similarityList = new ArrayList<UserSimilarityDAO>();
        ...
        for (int i = 0; i < userIdList.size() - 1; i++) {
            for (int j = i + 1; j < userIdList.size(); j++) {
                ...
                userSimilarityDAO.setUserId(userIdList.get(i));
                userSimilarityDAO.setUserRefId(userIdList.get(j));
                userSimilarityDAO.setSimilarity(similarity);
                // 将计算出的用户以及用户之间的相似度对象存入list集合
                similarityList.add(userSimilarityDAO);
            }
        }
     ...
     * @paramuserSimilarityDAO 用户相似度列表
     * @param topN 与userId相似用户的数量
     * @return 与usereId最相似的topN个用户
     */
    public static List<Long> getSimilarityBetweenUsers(Long userId, List<UserSimilarityDAO> userSimilarityDAOList, Integer topN) {
        ...
        PriorityQueue<UserSimilarityDAO> minHeap = new PriorityQueue<UserSimilarityDAO>(new Comparator<UserSimilarityDAO>(){
            @Override
            public int compare(UserSimilarityDAO o1, UserSimilarityDAO o2) {
        ...
        for (UserSimilarityDAO userSimilarityDAO : userSimilarityDAOList) {
            if (minHeap.size() < topN) {
                minHeap.offer(userSimilarityDAO);
                System.out.println(minHeap.peek().getSimilarity());
            } else if (minHeap.peek().getSimilarity() < userSimilarityDAO.getSimilarity()) {
                minHeap.poll();
                minHeap.offer(userSimilarityDAO);
            }
        }
        // 把得到的最大的相似度的用户的id取出来(不要取它自己)
        for (UserSimilarityDAO userSimilarityDAO : minHeap) {
            similarityList.add(userSimilarityDAO.getUserId() == userId ? userSimilarityDAO.getUserRefId() : userSimilarityDAO.getUserId());
        }
     ...
     */
    public static List<Long> getRecommendateCategory2(Long userId, List<Long> similarUserList, List<UserActiveDAO> userActiveList) {
        List<Long> recommeddateProductList = new ArrayList<Long>();
        
        // userId的浏览行为列表
        List<UserActiveDAO> userIdActiveList = findUsersBrowsBehavior(userId, userActiveList);
        
        // 对userId的浏览行为按照二级类目id排个序，方便后续与推荐的用户的浏览行为中的二级类目的点击次数直接相减，避免时间复杂度为O(n2)
        Collections.sort(userIdActiveList, new Comparator<UserActiveDAO>(){
            @Override
            public int compare(UserActiveDAO o1, UserActiveDAO o2) {
        ...
        for (Long refId : similarUserList) {
            ...
            // 找到当前这个用户的浏览行为
            List<UserActiveDAO> currActiveList = findUsersBrowsBehavior(refId, userActiveList);
            
            // 排序，同上述理由
            Collections.sort(currActiveList, new Comparator<UserActiveDAO>(){
                @Override
                public int compare(UserActiveDAO o1, UserActiveDAO o2) {
                    return o1.getCategory2Id().compareTo(o2.getCategory2Id());
                }
     ...
     * @return 当前用户的浏览行为列表
     */
    public static List<UserActiveDAO> findUsersBrowsBehavior(Long userId, List<UserActiveDAO> userActiveList) {
        List<UserActiveDAO> currActiveList = new ArrayList<UserActiveDAO>();
        for (UserActiveDAO userActiveDAO : userActiveList) {
            if (userActiveDAO.getUserId().equals(userId)) {
                currActiveList.add(userActiveDAO);
            }
        }
