...
import com.cj.shopping.sysmanage.controller.ProductController;
...
public class ProductServiceImpl implements ProductService {
	...
	@Override
	public List<ProductDAO> listProductByCategory2Id(Long category2Id) {
		
		Product product = new Product();
		product.setCategory2Id(category2Id);
		List<ProductDAO> productList = this.productMapper.listProduct(product);
		
		return productList;
	}
    
    @Override
    public PageInfo<ProductDAO> listProductPage(Product product, PageParam pageParam) {
        if (product == null) {
            return null;
        }
        ...
        List<ProductDAO> productList = this.productMapper.listProduct(product);
        // 3.封装分页信息
        PageInfo<ProductDAO> productInfo = new PageInfo<ProductDAO>(productList);
