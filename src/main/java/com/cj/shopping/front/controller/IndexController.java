...
import com.cj.shopping.sysmanage.service.Category2Service;
...
//		List<Category1DAO> category1DAOList = this.category1Service.listCategory1DAO(category1);
//		session.setAttribute("category1List", category1DAOList);
...
//		Long currU Id = 1L;
		...
		// 2.找到当前用户与其他用户的相似度列表
		List<UserSimilarityDAO> userSimilarityList = this.userSimilarityService.listUserSimilarityByUId(currUId);
	...
	public String findProductListByCategory1Id(HttpSession session, HttpServletRequest request,
		...
		List<Category1DAO> category1DAOList = (List<Category1DAO>) servletContext.getAttribute("category1List");
		if (category1DAOList == null) {
			Category1 category1 = new Category1();
			category1.setShowFlag(1);
			category1DAOList = this.category1Service.listCategory1DAO(category1);
			servletContext.setAttribute("category1List", category1DAOList);
		}*/
		...
		PageInfo<ProductDAO> productList = this.productService.listProductPage(product, pageParam);
		...
		List<Category2DAO> category2List = this.category2Service.listCategory2(category2);
		// 4.2记录当前用户对这些二级类目的浏览次数
		for (Category2DAO category2DAO : category2List) {
			UserActiveDAO userActiveDAO = new UserActiveDAO();
			userActiveDAO.setUserId(currUId);
			userActiveDAO.setCategory2Id(category2DAO.getCategory2Id());
			boolean flag = this.userActiveService.saveUserActive(userActiveDAO);
			if (flag) {
	...
	public String findProductListByCategory2Id(HttpSession session, HttpServletRequest request,
		...
		@SuppressWarnings("unchecked")
		List<Category1DAO> category1DAOList = (List<Category1DAO>) session.getAttribute("category1List");
		if (category1DAOList == null) {
			Category1 category1 = new Category1();
			category1.setShowFlag(1);
			category1DAOList = this.category1Service.listCategory1DAO(category1);
	...
	public String getProductDetail(HttpSession session, HttpServletRequest request,
		...
		UserActiveDAO userActiveDTO = new UserActiveDAO();
	...
	public void setPageAttribute(HttpServletRequest request, Long categoryId, PageInfo<ProductDAO> productList, String pageType) {
